...
class TranslationService {
  ...
  func fetchTranslation(translationUrl: String,
                        completion: @escaping (_ result: TranslationObject?,
    _ error: Error?) -> Void) {
  ...
  /// Method to auto detect the language from a text to translate.
  /// Can be combined with fetchTranslation to create an
  /// autodetected source language request to another
  ///
  /// - Parameters:
  ...
  func detectLanguage(from string: String, completion: @escaping(_ language: String, _ error: Error?) -> Void ) {
    ...
    let request = "\(Constants.Url.TRANSLATION_BASE_URL)\(TranslationService.API_KEY)&q=\(urlEncodedText)"
    var language: String?
    // Make the call async to separate from UI calls
    DispatchQueue.main.async {
